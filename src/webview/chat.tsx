...
import './styles/projectInputForm.css'
import { vs } from 'react-syntax-highlighter/dist/esm/styles/hljs'
interface ProjectInputFormProps {
  onBack: () => void; // 返回按钮的回调
}

const ProjectInputForm: React.FC<ProjectInputFormProps> = ({onBack}) => {
  const [projectDir, setProjectName] = useState<string>('');
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const message = {
      type: EVENT_NAME.twinnyGenerateProject,
      projectDir,        // 表单中的项目名称
      projectDescription,    // 表单中的项目语言
      projectLanguage          // 生成项目的目录
    };  
    global.vscode.postMessage(message);
  };
  return (
    <div style={{ padding: '20px', textAlign: 'center' }}>
      <h2 style={{ fontSize: '24px', marginBottom: '20px' }}>创建新项目</h2>
      <form onSubmit={handleSubmit} style={{ maxWidth: '400px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ marginBottom: '15px', textAlign: 'left' }}>
          <label style={{ fontWeight: 'bold', marginBottom: '5px', display: 'block' }}>
            ...
            <textarea 
              ...
              required 
              style={{ width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '4px', marginTop: '5px', minHeight: '80px' }}
            />
          </label>
        </div>
        <div style={{ marginBottom: '15px', textAlign: 'left' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '20px' }}>
          ...
          <VSCodeButton type="button" appearance="secondary" onClick={onBack}>返回</VSCodeButton>
        </div>
      </form>
    </div>
...
export const Chat = () => {
  ...
  const handleCompletionMessage = async (message: ServerMessage) => {
    if (stopRef.current) {
      generatingRef.current = false
      return
    }
  ...
  const handleNewButtonClick = () => {
    setShowInputForm(true); // 显示项目输入表单
  ...
  return (
    <VSCodePanelView>
       {showInputForm ? (
        <ProjectInputForm onBack={handleBack} />
      ) : (
