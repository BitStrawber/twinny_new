...
import {ProjectGenerator, ProjectGenerator} from './extension/providers/fileCreator'
...
export async function activate(context: ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(TWINNY_COMMAND_NAME.generateProject, async () => {
      sidebarProvider.webView?.onDidReceiveMessage(async (message) => {
        if (message.type === EVENT_NAME.twinnyGenerateProject) {
          const { projectDir, projectDescription, projectLanguage } = message;
          // 调用你的项目生成服务
          projectGenerator.callOllamaService(projectDescription, projectLanguage,projectDir);
        }
      });
    }),
